...
/// @title Canonical OptiFuseDEX  V3 factory
/// @notice Deploys OptiFuseDEX  V3 pools and manages ownership and control over pool protocol fees
contract OptiFuseV3Factory is IOptiFuseV3Factory {
    /// @inheritdoc IOptiFuseV3Factory
    ...
    function createPool(
        ...
        pool = IOptiFuseV3PoolDeployer(poolDeployer).deploy(address(this), token0, token1, fee, tickSpacing);
    ...
    function setFeeProtocol(address pool, uint32 feeProtocol0, uint32 feeProtocol1) external override onlyOwner {
        IOptiFuseV3Pool(pool).setFeeProtocol(feeProtocol0, feeProtocol1);
    }
    ...
    function collectProtocol(
        ...
        return IOptiFuseV3Pool(pool).collectProtocol(recipient, amount0Requested, amount1Requested);
    }

    function setLmPool(address pool, address lmPool) external override onlyOwnerOrLmPoolDeployer {
        IOptiFuseV3Pool(pool).setLmPool(lmPool);
    }
}
