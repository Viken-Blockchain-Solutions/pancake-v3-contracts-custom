...
import { TestOptiFuseV3SwapPay } from '../typechain-types/contracts/test/TestOptiFuseV3SwapPay'
...
import { TestOptiFuseV3ReentrantCallee } from '../typechain-types/contracts/test/TestOptiFuseV3ReentrantCallee'
...
describe('OptiFuseV3Pool', () => {
  ...
  let swapTarget: TestOptiFuseV3Callee
  ...
  async function initializeAtZeroTick(pool: MockTimeOptiFuseV3Pool): Promise<void> {
    await pool.initialize(encodePriceSqrt(1, 1))
  ...
  describe('#lock', () => {
    ...
    it('cannot reenter from swap callback', async () => {
      const reentrant = (await (
        await ethers.getContractFactory('TestOptiFuseV3ReentrantCallee')
      ).deploy()) as TestOptiFuseV3ReentrantCallee
  ...
  describe('swap underpayment tests', () => {
    let underpay: TestOptiFuseV3SwapPay
    beforeEach('deploy swap test', async () => {
      const underpayFactory = await ethers.getContractFactory('TestOptiFuseV3SwapPay')
      underpay = (await underpayFactory.deploy()) as TestOptiFuseV3SwapPay
