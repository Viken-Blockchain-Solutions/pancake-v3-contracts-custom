...
import '@optifusedex/v3-periphery/contracts/base/BlockTimestamp.sol';
import '@optifusedex/v3-periphery/contracts/libraries/Path.sol';
import '@optifusedex/v3-periphery/contracts/libraries/PoolAddress.sol';
import '@optifusedex/v3-core/contracts/interfaces/IOptiFuseV3Pool.sol';
import '@optifusedex/v3-periphery/contracts/libraries/OracleLibrary.sol';
...
abstract contract OracleSlippage is IOracleSlippage, PeripheryImmutableState, BlockTimestamp {
    ...
    function getBlockStartingAndCurrentTick(IOptiFuseV3Pool pool)
        internal
        view
    ...
    function getPoolAddress(
        ...
        address tokenB,
        uint24 fee
    ) internal view virtual returns (IOptiFuseV3Pool pool) {
        pool = IOptiFuseV3Pool(PoolAddress.computeAddress(deployer, PoolAddress.getPoolKey(tokenA, tokenB, fee)));
    }
        ...
        returns (int256 syntheticAverageTick, int256 syntheticCurrentTick)
    {
        ...
        for (uint256 i = 0; i < numPools; i++) {
            ...
            IOptiFuseV3Pool pool = getPoolAddress(tokenIn, tokenOut, fee);
            ...
            if (secondsAgo == 0) {
                ...
                (, currentTick, , , , , ) = IOptiFuseV3Pool(pool).slot0();
            }
