...
const artifacts: { [name: string]: ContractJson } = {
  ...
  OptiFuseInterfaceMulticall: require('../artifacts/contracts/lens/OptiFuseInterfaceMulticall.sol/OptiFuseInterfaceMulticall.json'),
...
async function main() {
  ...
  const OptiFuseV3PoolDeployer_address = deployedContracts.OptiFuseV3PoolDeployer
  const OptiFuseV3Factory_address = deployedContracts.OptiFuseV3Factory

  const SwapRouter = new ContractFactory(artifacts.SwapRouter.abi, artifacts.SwapRouter.bytecode, owner)
  const swapRouter = await SwapRouter.deploy(OptiFuseV3PoolDeployer_address, OptiFuseV3Factory_address, config.WNATIVE)

  // await tryVerify(swapRouter, [OptiFuseV3PoolDeployer_address, OptiFuseV3Factory_address, config.WNATIVE])
  ...
  const baseTokenUri = 'https://nft.OptiFuseswap.com/v3/'
  ...
  const OptiFuseInterfaceMulticall = new ContractFactory(
    artifacts.OptiFuseInterfaceMulticall.abi,
    artifacts.OptiFuseInterfaceMulticall.bytecode,
    owner
  )

  const optiFuseInterfaceMulticall = await OptiFuseInterfaceMulticall.deploy()
  console.log('OptiFuseInterfaceMulticall', optiFuseInterfaceMulticall.address)

  // await tryVerify(OptiFuseInterfaceMulticall)
  ...
  const contracts = {
    ...
    OptiFuseInterfaceMulticall: optiFuseInterfaceMulticall.address,
  }
